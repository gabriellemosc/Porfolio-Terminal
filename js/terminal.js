 ...
 */
function add_art(art, classname = "ascii-art") {
  let asciiContainer = document.querySelector(".ascii-container");

  // Se o contêiner ASCII ainda não existir, criá-lo
  if (!asciiContainer) {
    asciiContainer = document.createElement("div");
    asciiContainer.className = "ascii-container";
    app.appendChild(asciiContainer);
  }
  ...
  asciiContainer.innerHTML = ""; // Limpa conteúdo anterior
...
export async function get_input_command() {
  ...
  find: {
    ...
    if (value == "exit") {
      window.location.href = "https://devlucasborges.com";      /* TODO: COLOCAR MEU PORTFOLIO    */
      return;
    }
  ...
  switch (lang) {
    ...
    const p = document.createElement("p");
      ...
      document.getElementById("app").appendChild(p);
        // Adiciona a arte ASCII primeiro
        add_art(ascii_cubes, "ascii-art");
    ...
    const p_en = document.createElement("p");
      p_en.className = "pre_line"; // Classe opcional para estilização
      ...
      await delay(100);
      await add_text(
        'Run "<span class="highlight">lang"</span> to change the language | <span class="highlight">pt-br</span> / <span class="highlight">en-us</span><br><br>Run "<span class="highlight">help</span>" for a list of commands'
      );
      break;
  }
}
